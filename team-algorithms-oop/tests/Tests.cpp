...
TEST_CASE("Red Black Tree", "[RedBlackTree]") {
    ...
    SECTION("Constructor's from iterators") {
        auto rbTree = new tree::RedBlackTree<int>(elems.begin(), elems.end());
    ...
    SECTION("Constructor's from initialize list") {
        auto tree = new tree::RedBlackTree<int>{elems[0], elems[1], elems[2], elems[3], elems[4]};
    ...
    SECTION ("Deep copy") {
        auto tree = new tree::RedBlackTree<int>(*rbTree);
